...
export class IdentityController {
  ...
  static createUser = asyncHandler(async (req: Request, res: Response) => {
    ...
    // Create wallet (supports both Blockradar and local)
    const network = process.env.DEFAULT_NETWORK || 'base';
    const walletResult = await IdentityController.walletService.createWallet(hashedPassword, network);
    ...
    const user = new User({
      ...
      blockradarAddressId: walletResult.blockradarAddressId,
      blockradarNetwork: walletResult.network || network,
      ...
      try {
        ...
        const response: ApiResponse = {
          ...
          data: {
            ...
            network: user.blockradarNetwork || 'base',
            provider: 'blockradar'
          }
        };
        ...
        logger.error('Error getting Blockradar wallet balance:', error);
        return res.status(500).json({
          success: false,
