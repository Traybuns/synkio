...
This document outlines the updates made to implement an anonymous-first user experience for Synkio, where users can browse vendors without authentication but must sign in for purchases, wallet management, and vendor registration.
...
{
  email: string;
  username: string;  // e.g., "johndoe" (displayed as "johndoe.synkio")
...
- Hero section with tagline "Conversational Market | Let's Transact together securely"
...
**New Fields**:
- `username`: Required for new users, shown as "johndoe.synkio"
...
1. User visits Synkio home screen (anonymous by default)
...
### Username.synkio Identity

- All users have a unique username
- Username is displayed with `.synkio` suffix (e.g., `johndoe.synkio`)
...
3. Add username change functionality (keeping .synkio suffix)
